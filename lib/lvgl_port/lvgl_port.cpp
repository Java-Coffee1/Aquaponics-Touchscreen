...
{
    return get_next_frame_buffer((esp_lcd_panel_handle_t)panel_handle); // Cast void* to esp_lcd_panel_handle_t
}
...
static void flush_dirty_copy(void *dst, void *src, lv_port_dirty_area_t *dirty_area)
{
    ...
    for (int i = 0; i < dirty_area->inv_p; i++) {
        ...
        if (dirty_area->inv_area_joined[i] == 0) {
            ...
            // Cast void* pointers to uint16_t* for rotate_copy_pixel function
